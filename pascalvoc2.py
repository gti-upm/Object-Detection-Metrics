...
for metricsPerClass in detections:
    ...
    if totalPositives > 0:
        ...
        f.write('\nPrecision: %s' % prec)
        f.write('\nRecall: %s' % rec)
        mPrec = statistics.mean(precision)
        f.write('\nMean Precision: %s' % mPrec)
        mRec = statistics.mean(recall)
        f.write('\nMean Recall: %s' % mRec)
        f1_score = 2 * (mPrec * mRec) / (mPrec + mRec)  # F1-score = 2 * (precision * recall) / (precision + recall)
        f.write('\nMean F1-score: %s' % f1_score)
